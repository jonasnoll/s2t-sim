...
class Modeling():
    ...
    def run_modeling(self, train_dataset, test_dataset, model_id):
        ...
        print(f"Hyperparameter: model={model_id}, num_epochs={self.num_epochs}, batch_size={self.batch_size}, learning_rate={self.learning_rate}, shuffle_train_loader={self.shuffle_train_loader}, datalader_workers={self.datalader_workers}")
        if self.log:
            self.log.info(
    ...
    def train_model(self, train_loader):
        ...
        for epoch in range(self.num_epochs):
            for i, (images, labels) in enumerate(train_loader):
                # print(f"labels: {labels.shape} - min {min(labels)} - max {max(labels)}")
                ...
                # if (i+1) % int(math.ceil(n_total_steps/4)) == 0 or (i+1) % n_total_steps == 0:
                if (i+1) % n_total_steps == 0:
                    print(
                        f'Epoch [{epoch+1}/{self.num_epochs}], Step [{i+1}/{n_total_steps}], Loss: {loss.item():.4f}')

            self.metrics['train_loss'].append(loss.item())
            # apply scheduler with new epoch
            # scheduler.step()

        tr_end = time.time()
        print(f"# Training Successful - took {timer(tr_start, tr_end)}#")
        if self.log:
    ...
    def test_model(self, test_loader):
        ...
        with torch.set_grad_enabled(False):
            n_correct = 0
            n_samples = 0
            # n_class_correct = [0 for i in range(self.n_classes)]
            # n_class_samples = [0 for i in range(self.n_classes)]
            for images, labels in test_loader:
                ...
                n_samples += labels.size(0)  # adds batch size on top
                n_correct += (predicted == labels).sum().item()

                # This is for looking at what classes were predicted wrong
            ...
            print(f'Accuracy of the Network on Testdata: {acc:.4f} %\n')
            if self.log:
